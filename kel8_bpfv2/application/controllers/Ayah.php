...
class Ayah extends CI_Controller
{
    ...
    {
        $data['judul'] = "Halaman Data Ayah";
        $data['user'] = $this->db->get_where('user', ['email' => $this->session->userdata('email')])->row_array();
        $data['ayah'] = $this->Ayah_model->get();
        $this->load->view("layout/header", $data);
        $this->load->view("ayah/vw_ayah", $data);
        $this->load->view("layout/footer", $data);
    }

    public function tambah()
    {
        $data['judul'] = "Halaman Tambah Data Ayah";
        $data['user'] = $this->db->get_where('user', ['email' => $this->session->userdata('email')])->row_array();
        ...
        if ($this->form_validation->run() == false) {
            $this->load->view("layout/header", $data);
            $this->load->view("ayah/vw_tambah_ayah", $data);
            $this->load->view("layout/footer", $data);
        } else {
            $data = [
    ...
    {
        $data['judul'] = "Halaman Edit Data Ayah";
        $data['user'] = $this->db->get_where('user', ['email' => $this->session->userdata('email')])->row_array();
        $data['ayah'] = $this->Ayah_model->getById($id);
        $this->form_validation->set_rules('nik', 'NIK', 'required', [
            'required' => 'NIK Wajib di isi'
        ]);
        $this->form_validation->set_rules('nama', 'Nama', 'required', [
        ...
        if ($this->form_validation->run() == false) {
            $this->load->view("layout/header", $data);
            $this->load->view("ayah/vw_ubah_ayah", $data);
            ...
            $this->Ayah_model->update(['nik' => $id], $data);
            $this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">Data Ayah Berhasil
			Diubah!</div>');
            redirect('Ayah');
        }
    }
    ...
    public function hapus($id)
    {
        ...
        if ($error['code'] != 0) {
			...
			fas fa-info-circle"></i>Data Ayah tidak dapat dihapus (sudah berelasi)!</div>');
        } else {
            $this->session->set_flashdata('message', '<div class="alert alert-success" role="alert"><i
			class="icon fas fa-check-circle"></i>Data Ayah Berhasil Dihapus!</div>');
        }
        redirect('Ayah');
    }
}
